...
case ${1} in
  ...
  CoreLab_TVPro)
    DEVICE_CFGLOAD="Generic_cfgload"  # used for generic image
    DEVICE_BOOT_LOGO="corelab_tvpro-boot-logo.bmp.gz"
    DEVICE_DTB="device_trees/t7_a311d2_corelab_tvpro.dtb" # Copy to dtb.img, multi dtb format, defined in in projects/Amlogic-ce/devices/Amlogic-no/bootloader/dtb.conf
    DEVICE_UBOOT="${1}_u-boot"
    DEVICE_BOOT_INI="${1}_boot.ini"
    [ -n "${2}" ] && DEVICE_UBOOT_BIN="$(get_build_dir u-boot-${1})/build/u-boot.bin.sd.bin.signed"
  ;;
